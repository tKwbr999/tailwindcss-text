...
function ArticlePage() {
  const { section: sectionParam, articleName: articleNameParam } = useParams<{ section: string; articleName: string }>();
  ...
  useEffect(() => {
    ...
    for (const section of sections) {
      ...
      const formattedSectionParam = sectionParam?.toLowerCase().replace(/^\d+_/, '');
      const formattedSectionName = section.name.toLowerCase().replace(/^\d+_/, '');
      ...
      if (formattedSectionName === formattedSectionParam) {
        foundArticle = section.articles.find(article => {
          ...
          const formattedArticleNameParam = articleNameParam?.toLowerCase().replace(/^\d+_/, '').replace(/\.md$/, '');
          const formattedArticleName = article.name.toLowerCase().replace(/^\d+_/, '').replace(/\.md$/, '').replace(/-/g, ' '); // Also replace hyphens in name for comparison
          // Handle potential differences in formatting (e.g., spaces vs hyphens)
          const articleNameFromPath = article.path.split('/').pop()?.toLowerCase().replace(/^\d+_/, '').replace(/\.md$/, '');
          return formattedArticleName.replace(/ /g, '') === formattedArticleNameParam?.replace(/ /g, '') ||
                 articleNameFromPath?.replace(/-/g, '') === formattedArticleNameParam?.replace(/-/g, '');
        });
    ...
    if (foundArticle) {
      ...
      fetch(`/${foundArticle.markdownPath}`) // Fetch from the root, assuming files are served there
        ...
        .catch(e => {
          ...
          setError(`Failed to load article content from ${foundArticle?.markdownPath}. Please check the file path and server configuration.`);
          setLoading(false);
        });
    } else {
      setError(`Article "${articleNameParam}" in section "${sectionParam}" not found.`);
      setLoading(false);
    }
