...
const InlineBlockExample: React.FC = () => {
  return (
    <div>
      テキストの途中にある
      <span className="inline-block bg-green-200 dark:bg-green-900 p-2 rounded mx-1">inline-block</span>
      要素。幅と高さ、マージン/パディングが適用されます。
      <span className="inline-block bg-green-200 dark:bg-green-900 p-4 rounded mx-1">別の inline-block</span>
    </div>
  );
};
...
const GridExample: React.FC = () => {
  return (
    <div className="grid grid-cols-3 gap-2 bg-teal-200 dark:bg-teal-900 p-2 rounded">
      ...
      <div className="bg-teal-400 dark:bg-teal-700 p-2 rounded col-span-2">Grid Item 4 (span 2)</div>
...
const ContentsExample: React.FC = () => {
    ...
    return (
      <div className="flex space-x-4 text-center">
        ...
        <div className="contents"> {/* このDiv自体はレイアウトに影響しない */}
...
const HiddenExample: React.FC = () => {
  return (
    <div>
      ...
      <div className="hidden bg-red-200 dark:bg-red-900 p-2 rounded my-2">この要素は非表示です (display: none)。</div>
      ...
      <div className="max-w-4xl mx-auto bg-stone-100 dark:bg-stone-800 rounded-lg shadow-md p-6 space-y-8">
        ...
        <Card>
          ...
          <CardContent className="text-stone-700 dark:text-stone-300 space-y-4">
            <p>
              要素の表示タイプ (外側の表示タイプと内側の表示タイプ) を制御するためのユーティリティクラスです。要素がどのようにレイアウトフローに参加し、子要素をどのように配置するかを決定します。
            </p>
            <p>
        ...
        <Card>
          ...
          <CardContent className="text-stone-700 dark:text-stone-300 space-y-4">
            ...
            <ul className="list-disc list-inside space-y-1 pl-4 columns-2">
              <li><strong><code>block</code></strong>: ブロックレベル要素。前後に改行が入り、幅と高さを持ちます。</li>
              <li><strong><code>inline-block</code></strong>: インラインブロック要素。テキストのようにインラインに配置されますが、幅、高さ、垂直マージン/パディングを持ちます。</li>
              <li><strong><code>inline</code></strong>: インライン要素。テキストの一部として扱われ、幅/高さや垂直マージン/パディングは通常適用されません。</li>
              <li><strong><code>flex</code></strong>: ブロックレベルのフレックスコンテナ。子要素をフレックスアイテムとして配置します。</li>
              <li><strong><code>inline-flex</code></strong>: インラインレベルのフレックスコンテナ。</li>
              <li><strong><code>grid</code></strong>: ブロックレベルのグリッドコンテナ。子要素をグリッドアイテムとして配置します。</li>
              <li><strong><code>inline-grid</code></strong>: インラインレベルのグリッドコンテナ。</li>
              <li><strong><code>contents</code></strong>: 要素自体はレイアウトツリーから削除され、子要素が親要素に直接属するかのように扱われます (注意が必要)。</li>
              <li><strong><code>table</code></strong>, <strong><code>inline-table</code></strong>, <strong><code>table-caption</code></strong>, <strong><code>table-cell</code></strong>, <strong><code>table-column</code></strong>, <strong><code>table-column-group</code></strong>, <strong><code>table-footer-group</code></strong>, <strong><code>table-header-group</code></strong>, <strong><code>table-row-group</code></strong>, <strong><code>table-row</code></strong>: テーブルレイアウト関連。</li>
              <li><strong><code>flow-root</code></strong>: 新しいブロック整形コンテキスト (BFC) を確立します。フロートのクリアなどに使われます。</li>
              <li><strong><code>hidden</code></strong>: 要素を非表示にします (<code>display: none;</code>)。レイアウトにも影響を与えません。</li>
            </ul>
          </CardContent>
        </Card>
             ...
             <div>
              ...
              <div className="p-4 border border-stone-300 dark:border-stone-600 rounded">
               ...
               <p className="text-sm text-stone-600 dark:text-stone-400 mt-2">
                注意: <code>contents</code> を適用した要素 (中央の2つのオレンジのボックスの親) はレイアウト上存在しないかのように扱われ、子要素が直接親の Flexbox (または Grid) レイアウトに参加します。アクセシビリティへの影響に注意が必要です。
              </p>
            </div>
             {/* Table (簡易例) */}
             <div>
              <h3 className="font-semibold text-stone-700 dark:text-stone-300 mb-2">table (関連クラス)</h3>
              <pre className="bg-stone-200 dark:bg-stone-700 p-2 rounded overflow-x-auto text-sm text-stone-800 dark:text-stone-200 mb-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription className="text-stone-600 dark:text-stone-400">
              ブレークポイントプレフィックス (<code>sm:</code>, <code>md:</code> など) を使って、特定の画面サイズでのみ <code>display</code> プロパティを変更できます。これはレスポンシブデザインで非常に一般的です。
            </CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-stone-700 dark:text-stone-300 mb-4">
              例えば、<code>block md:flex</code> はデフォルトでブロック要素として表示し、中程度の画面サイズ以上でフレックスコンテナとして表示します。<code>hidden md:block</code> はモバイルで非表示にし、中画面以上で表示するためによく使われます。
            </p>
            {/* コード表示 */}
...
<div class="hidden lg:grid ...">...</div>
        ...
        <Card>
          ...
          <CardContent className="text-stone-700 dark:text-stone-300 space-y-4">
            <ul className="list-disc list-inside space-y-1 pl-4">
              <li>
                <code>display</code> プロパティは要素の基本的なレイアウト挙動を決定するため、他のレイアウト関連ユーティリティ (Flexbox, Grid, Position など) と密接に関連します。
              </li>
              <li>
                <code>hidden</code> (<code>display: none;</code>) は要素を完全に非表示にし、レイアウト上のスペースも占有しません。要素を視覚的に隠しつつスペースは維持したい場合は <code>invisible</code> (<code>visibility: hidden;</code>) を使用します。
              </li>
              <li>
        ...
        <Card>
          ...
          <CardContent className="text-stone-700 dark:text-stone-300 space-y-4">
            <ul className="list-disc list-inside space-y-1 pl-4">
               <li>
                Flexbox 関連 (<code>flex-direction</code>, <code>justify-content</code>, <code>align-items</code> など): <code>flex</code> または <code>inline-flex</code> と組み合わせて使用します。
              </li>
              <li>
                Grid 関連 (<code>grid-template-columns</code>, <code>grid-template-rows</code>, <code>gap</code> など): <code>grid</code> または <code>inline-grid</code> と組み合わせて使用します。
              </li>
              <li>
                <code>visibility</code> (<code>visible</code>, <code>invisible</code>): 要素の可視性を制御します (スペースは維持されます)。
              </li>
               <li>
                <code>position</code> (<code>static</code>, <code>relative</code>, <code>absolute</code>, <code>fixed</code>, <code>sticky</code>): 要素の配置方法を制御します。
              </li>
            </ul>
          </CardContent>
        </Card>
